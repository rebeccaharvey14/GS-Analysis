...
import time
sys.path.append('/home/rharvey/GS/GS_FluxRopeDetectionPackage')
import MyPythonPackage.fluxrope_intervals as FR
...
def plotFluxRopeInfo(oneRecord_DF):
    ...
    phi_deg = round(oneRecord_DF['phi_deg'].iloc[0])
    ...
    Vswlambda1 = oneRecord_DF['Vswlambda1'].iloc[0]
    Vswlambda2 = oneRecord_DF['Vswlambda2'].iloc[0]
    Vswlambda3 = oneRecord_DF['Vswlambda3'].iloc[0]
    VsweigenVectorMaxVar_lambda1 = np.array([oneRecord_DF['VsweigenVectorMaxVar_lambda1[0]'].iloc[0], oneRecord_DF['VsweigenVectorMaxVar_lambda1[1]'].iloc[0], oneRecord_DF['VsweigenVectorMaxVar_lambda1[2]'].iloc[0]])
    VsweigenVectorInterVar_lambda2 = np.array([oneRecord_DF['VsweigenVectorInterVar_lambda2[0]'].iloc[0], oneRecord_DF['VsweigenVectorInterVar_lambda2[1]'].iloc[0], oneRecord_DF['VsweigenVectorInterVar_lambda2[2]'].iloc[0]])
    VsweigenVectorMinVar_lambda3 = np.array([oneRecord_DF['VsweigenVectorMinVar_lambda3[0]'].iloc[0], oneRecord_DF['VsweigenVectorMinVar_lambda3[1]'].iloc[0], oneRecord_DF['VsweigenVectorMinVar_lambda3[2]'].iloc[0]])
    Vrlambda1 = oneRecord_DF['Vrlambda1'].iloc[0]
    Vrlambda2 = oneRecord_DF['Vrlambda2'].iloc[0]
    Vrlambda3 = oneRecord_DF['Vrlambda3'].iloc[0]
    VreigenVectorMaxVar_lambda1 = np.array([oneRecord_DF['VreigenVectorMaxVar_lambda1[0]'].iloc[0], oneRecord_DF['VreigenVectorMaxVar_lambda1[1]'].iloc[0], oneRecord_DF['VreigenVectorMaxVar_lambda1[2]'].iloc[0]])
    VreigenVectorInterVar_lambda2 = np.array([oneRecord_DF['VreigenVectorInterVar_lambda2[0]'].iloc[0], oneRecord_DF['VreigenVectorInterVar_lambda2[1]'].iloc[0], oneRecord_DF['VreigenVectorInterVar_lambda2[2]'].iloc[0]])
    VreigenVectorMinVar_lambda3 = np.array([oneRecord_DF['VreigenVectorMinVar_lambda3[0]'].iloc[0], oneRecord_DF['VreigenVectorMinVar_lambda3[1]'].iloc[0], oneRecord_DF['VreigenVectorMinVar_lambda3[2]'].iloc[0]])
    A_range  = oneRecord_DF['A_range'].iloc[0]
    Pt_coeff = oneRecord_DF['Pt_coeff'].iloc[0]
    Path_length = oneRecord_DF['Path_length'].iloc[0]
    ...
    #print(GS_fluxRopeCandidate_DF)
    matrix_transToMVAB = np.array([eigenVectorMaxVar_lambda1, eigenVectorInterVar_lambda2, eigenVectorMinVar_lambda3]).T
    matrix_transToMVAVsw = np.array([VsweigenVectorMaxVar_lambda1, VsweigenVectorInterVar_lambda2, VsweigenVectorMinVar_lambda3]).T
    matrix_transToMVAVr = np.array([VreigenVectorMaxVar_lambda1, VreigenVectorInterVar_lambda2, VreigenVectorMinVar_lambda3]).T
    ...
    B_inGSE = GS_fluxRopeCandidate_DF.loc[:,['Bx', 'By', 'Bz']] # Produce a reference.
    # Get the solar wind slice.
    Vsw_inGSE = GS_fluxRopeCandidate_DF.loc[:,['Vx', 'Vy', 'Vz']] # Produce a reference.
    # Get the proton number density slice.
    Np_inGSE = GS_fluxRopeCandidate_DF.loc[:,['Np']] # Produce a reference.
    # Get the proton temperature slice. In Kelvin.
    Tp_inGSE = GS_fluxRopeCandidate_DF.loc[:,['Tp']] # Produce a reference.
    ...
    if 'Te' in GS_fluxRopeCandidate_DF.keys():
        ...
        Te_inGSE = GS_fluxRopeCandidate_DF.loc[:,['Te']] # Produce a reference.
    ...
    Vsw_inMVAVsw = Vsw_inGSE.dot(matrix_transToMVAVsw)

    Vr_inGSE = Vsw_inGSE - VHT_inGSE
    Vr_inMVAVr = Vr_inGSE.dot(matrix_transToMVAVr)
    Vr_inMVAB = Vr_inGSE.dot(matrix_transToMVAB)
    ...
    Pp = np.array(Np_inGSE['Np']) * 1e6 * 1.3806488e-23 * 1e9 * np.array(Tp_inGSE['Tp'])
    P_B = np.array((B_inFR[2] * 1e-9)**2 / (2.0*mu0) * 1e9) # 1e9 convert unit form pa to npa.
    Pt = Pp + P_B
    # Pt = np.array((B_inFR[2] * 1e-9)**2 / (2.0*mu0) * 1e9) # 1e9 convert unit form pa to npa.
    # Find the index of turnPoint.
    index_turnTime = B_inFR.index.get_loc(turnTime,method='nearest')
    ...
    Np_sub1 = Np_inGSE[:index_turnTime+1]
    Np_sub2 = Np_inGSE[index_turnTime:]
    Tp_sub1 = Tp_inGSE[:index_turnTime+1]
    Tp_sub2 = Tp_inGSE[index_turnTime:]
    ...
    time_str = str(startTime.strftime('%Y-%m-%d %H:%M:%S'))+' ~ '+str(endTime.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    plot_Pt_vs_A.legend(loc='best',prop={'size':7})
    ...
    VA_inMVAB = np.array(B_inMVAB * 1e-9) / np.sqrt(mu0 * P_massDensity_array) / 1000.0
    plot_Hodogram_B2_B1 = ax1[0][1]
    plot_Hodogram_B2_B1.set_aspect('equal')
    plot_Hodogram_B2_B1.set_title('Hodogram(V2_V1 MVAB frame)',fontsize=fig_title_fontsize)
    #plot_Hodogram_B2_B1.plot(B_inMVAB[1], B_inMVAB[0], color = 'blue', marker='.')
    plot_Hodogram_B2_B1.plot(VA_inMVAB[:,1], VA_inMVAB[:,0], color = 'blue', marker='.',label='B/$\sqrt{\mu_0 \\rho}$')
    plot_Hodogram_B2_B1.plot(Vr_inMVAB[1], Vr_inMVAB[0], color = 'red', marker='.',label='V$_{remaining}$')
    plot_Hodogram_B2_B1.legend(loc='upper right',prop={'size':7})
    lm1=round(lambda1/lambda2,2)
    lm=str(lm1)
    plt.figtext(0.38,0.57,'$\lambda_1/\lambda_2 \\approx$',fontsize=9)
    plt.figtext(0.42,0.57,lm,fontsize=9)
    plot_Hodogram_B2_B1.grid()
    ...
    plot_Hodogram_B2_B1.set_xlabel('V2(km/s)',fontsize=fig_xlabel_fontsize)
    plot_Hodogram_B2_B1.set_ylabel('V1(km/s)',fontsize=fig_ylabel_fontsize)
    ...
    plot_Hodogram_B3_B1.set_title('Hodogram(V3_V1 MVAB frame)', fontsize=fig_title_fontsize)
    #plot_Hodogram_B3_B1.plot(B_inMVAB[2], B_inMVAB[0], color = 'blue', marker='.')
    plot_Hodogram_B3_B1.plot(VA_inMVAB[:,2], VA_inMVAB[:,0], color = 'blue', marker='.',label='B/$\sqrt{\mu_0 \\rho}$')
    plot_Hodogram_B3_B1.plot(Vr_inMVAB[2], Vr_inMVAB[0], color = 'red', marker='.',label='V$_{remaining}$')
    plot_Hodogram_B3_B1.legend(loc='upper right',prop={'size':7})
    ...
    plot_Hodogram_B3_B1.set_xlabel('V3(km/s)',fontsize=fig_xlabel_fontsize)
    plot_Hodogram_B3_B1.set_ylabel('V1(km/s)',fontsize=fig_ylabel_fontsize)
    ...
    lm2=round(lambda1/lambda3,2)
    lmm=str(lm2)
    plt.figtext(0.7,0.57,'$\lambda_1/\lambda_3 \\approx$',fontsize=9)
    plt.figtext(0.74,0.57,lmm,fontsize=9)
    plot_Hodogram_B3_B1.grid()    
    ...
    plot_Hodogram_Bz_By.set_title('Hodogram (V2_V1 MVAV$_{remaining}$ frame)', fontsize=fig_title_fontsize)
    plot_Hodogram_Bz_By.plot(Vr_inMVAVr[1], Vr_inMVAVr[0], color = 'blue', marker='.',label='V$_{remaining}$')
    ...
    plot_Hodogram_Bz_By.set_xlabel('V2(km/s)',fontsize=fig_xlabel_fontsize)
    plot_Hodogram_Bz_By.set_ylabel('V1(km/s)',fontsize=fig_ylabel_fontsize)
    ...
    lm3=round(Vrlambda1/Vrlambda2,2)
    lmmm=str(lm3)
    plt.figtext(0.38,0.09,'$\lambda_1/\lambda_2 \\approx$',fontsize=9)
    plt.figtext(0.42,0.09,lmmm,fontsize=9)
    plot_Hodogram_Bz_By.grid()
    
    # Plot Hodogram_Bx_By:
    plot_Hodogram_Bx_By = ax1[1][2]
    plot_Hodogram_Bx_By.set_aspect('equal', adjustable='box')
    plot_Hodogram_Bx_By.set_title('Hodogram (V3_V1 MVAV$_{remaining}$ frame)', fontsize=fig_title_fontsize)
    plot_Hodogram_Bx_By.plot(Vr_inMVAVr[2], Vr_inMVAVr[0], color = 'blue', marker='.',label='V$_{remaining}$')
    ...
    plot_Hodogram_Bx_By.set_xlabel('V3(km/s)',fontsize=fig_xlabel_fontsize)
    plot_Hodogram_Bx_By.set_ylabel('V1(km/s)',fontsize=fig_ylabel_fontsize)
    ...
    lm4=round(Vrlambda1/Vrlambda3,2)
    lmmmm=str(lm4)
    plt.figtext(0.7,0.09,'$\lambda_1/\lambda_3 \\approx$',fontsize=9)
    plt.figtext(0.74,0.09,lmmmm,fontsize=9)
    plot_Hodogram_Bx_By.grid()
    ...
    fig2,ax2 = plt.subplots(6,1, figsize=(12, 14))
    plot_B_GSE = ax2[0]
    plot_B_FR = ax2[1]
    #e_pitch = ax2[2]
    plot_Vsw_GSE = ax2[2]
    ...
    # Plot plot_B_GSE.
    plot_B_GSE.set_title('Magnetic Field (RTN Frame)', fontsize=fig_title_fontsize)
    ...
    # Plot plot_Vsw_GSE.
    plot_Vsw_GSE.set_title('Solar Wind Velocity (RTN Frame)', fontsize=fig_title_fontsize)
    plot_Vsw_GSE.xaxis.set_major_formatter(fig_formatter) # Set xlabel format.
    # Plot solar wind speed in GSE frame.
    plot_Vsw_GSE.plot(Vsw_inGSE.index, Vsw_inGSE['Vx'], color = 'r', label='Vr', linewidth=fig_linewidth, linestyle='solid')
    plot_Vsw_GSE.plot(Vsw_inGSE.index, Vsw_inGSE['Vy'], color = 'g', label='Vt', linewidth=fig_linewidth, linestyle='solid')
    plot_Vsw_GSE.plot(Vsw_inGSE.index, Vsw_inGSE['Vz'], color = 'b', label='Vn', linewidth=fig_linewidth, linestyle='solid')
    ...
    plot_Beta_p.set_ylabel(r'$\beta_p$',fontsize=fig_ylabel_fontsize)
    #plot_Beta_p.set_ylim([0,Beta_pmax])
    ...
    #Scax = fig2.add_axes([box.xmax + pad, box.ymin, width, box.height]) # Set colorbar position.
...
def setRootDir(ENV):
    return {
        'macbook'    : '/Users/rharvey/Documents/Research/GS/',
	'aurora'     : '/home/rharvey/GS/',
...
# Parameters.
dt = float(sys.argv[3]) # For 1 minute resolution data, the time increment is 60 seconds.
...
for year in range(year_start,year_end+1,1):
    ...
    duration_range_str = str(specified_startTime.strftime('%Y%m%d%H%M%S')) + '~' + str(specified_endTime.strftime('%Y%m%d%H%M%S'))


    # Specify input and output path
    rootDir = setRootDir('aurora') # 'macbook', 'bladerunner', or 'blueshark'.
    inputListDir = 'GS_SearchResult/selected_events/'
    inputFileName = '(|B|>=5)'+year_str+'_selected_events.p'
    ...
    print('Reading data...')
    GS_oneYearData_DataFrame = pd.read_pickle(open(rootDir + 'GS_DataPickleFormat/preprocessed/GS_' + year_str + '_AllData_DataFrame_preprocessed_THM.p','rb'))
